...
// Mengambil detail foto berdasarkan fotoID dan UserID
$query = "SELECT * FROM foto WHERE FotoID = ? AND UserID = ?";
...
if ($result->num_rows === 0) {
    die("Anda tidak diizinkan untuk mengedit foto ini.");
}

$foto = $result->fetch_assoc();


// Proses menangani edit
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['LokasiFile']) && $_FILES['LokasiFile']['error'] === UPLOAD_ERR_OK) {
        ...
        // Nama file baru untuk menghindari kesalahan
        ...
        if (move_uploaded_file($fileTmpPath, $dest_path)) {
            $query = "UPDATE foto SET JudulFoto = ?, DeskripsiFoto = ?, LokasiFile = ? WHERE FotoID = ? AND UserID = ?";
            $stmt = $kon->prepare($query);
            $stmt->bind_param('sssii', $judulFoto, $deskripsiFoto, $dest_path, $fotoID, $userID);
        } else {
            die('Error: File upload failed.');
        }
    } else {
        // Mengubah info foto tanpa mengubah file
        $query = "UPDATE foto SET JudulFoto = ?, DeskripsiFoto = ? WHERE FotoID = ? AND UserID = ?";
        $stmt = $kon->prepare($query);
        $stmt->bind_param('ssii', $judulFoto, $deskripsiFoto, $fotoID, $userID);
    }
...
<div class="container">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <br>
        <button href="dashboard.php" class="button">Kembali</a>
    </form>
</div>
</body>
</html>
